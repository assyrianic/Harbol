...
void test_harbol_map(FILE *const debug_stream) {
	...
	*/
	...
	for( size_t l=0; l < 8; l++ ) {
		int const n = rand() % 1000000;
		char x[10] = {0};
		int const len = sprintf(&x[0], "%d", n);
		harbol_map_insert(p, &x[0], len+1, &n, sizeof n);
	}
	
	size_t largest_len = 0;
	for( size_t n=0; n<p->len; n++ ) {
		struct const HarbolArray *bucket = p->buckets + n;
	...
	*/
	...
	for( size_t n=0; n < p->len; n++ ) {
		fprintf(debug_stream, "key: '%s', keylen: '%zu' - value == %" PRIi64 "\n", p->keys[n], p->keylens[n], (( union Value const* )p->datum[n])->int64);
	}
	
	/// test retrieval.
	fputs("\nmap :: test data retrieval.\n", debug_stream);
	fprintf(debug_stream, "ptr[\"1\"] == %" PRIi64 "\n", (( union Value const* )harbol_map_key_get(p, "1", sizeof "1"))->int64);
	fprintf(debug_stream, "ptr[\"2\"] == %" PRIi64 "\n", (( union Value const* )harbol_map_key_get(p, "2", sizeof "2"))->int64);
	fprintf(debug_stream, "stk[\"1\"] == %" PRIi64 "\n", (( union Value const* )harbol_map_key_get(&i, "1", sizeof "1"))->int64);
	...
	for( size_t n=0; n < i.len; n++ ) {
		fprintf(debug_stream, "key '%s' - value == %" PRIi64 "\n", i.keys[n], (( union Value const* )i.datum[n])->int64);
	}
	/// test setting.
	...
	fputs("\n", debug_stream);
	for( size_t n=0; n < p->len; n++ ) {
		fprintf(debug_stream, "key '%s' - value == %" PRIi64 "\n", p->keys[n], (( union Value const* )p->datum[n])->int64);
	}
