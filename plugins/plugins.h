...
HARBOL_EXPORT NEVER_NULL(1,2) bool harbol_plugin_load(struct HarbolPlugin *pl, char const path[], HarbolPluginEvent load_fn, void *userdata);
...
HARBOL_EXPORT NO_NULL NONNULL_RET char const *harbol_plugin_get_err(struct HarbolPlugin const *pl);

HARBOL_EXPORT NO_NULL void *harbol_plugin_get_obj(struct HarbolPlugin *pl, char const name[]);


typedef void HarbolPluginDirEvent(char const pl_path[], char const pl_name[], void *userdata);

HARBOL_EXPORT NEVER_NULL(1) bool harbol_plugin_dir_open(char const dir[], HarbolPluginDirEvent dir_fn, void *userdata, char const custom_ext[]);
